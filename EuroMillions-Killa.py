...
def x():
    sampleList = random.sample(range(1, 51),5)
    print(sampleList)
...
def y():
    sampleList = random.sample(range(1, 13),2)
    print(sampleList)
    sampleRandomNumberListAsString = ', '.join(str(y) for y in sampleList)
    output.delete(1.0, END)
    output.insert(END, sampleRandomNumberListAsString)
...
l1 = Label(window, text="          EuroMillions Killa", font="bold")
...
output = Text(GUIFrame1, width=19, height=1)
...
Button(GUIFrame2, text="Generate Lucky Stars", width=17, command = y).grid(row=5, column=1, sticky=W)
